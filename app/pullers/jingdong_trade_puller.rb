class JingdongTradePuller
  class << self
    def create(start_time = nil, end_time = nil)
      ...
      response = JingdongFu.get(method: '360buy.order.search', order_state:'WAIT_SELLER_STOCK_OUT', page:1, page_size: 10)
      ...
      (1..total_pages).each do |page|
        response = JingdongFu.get(method: '360buy.order.search', order_state:'WAIT_SELLER_STOCK_OUT', page: page, page_size: 10)
        # get(method: '360buy.order.search',
        # start_date: start_time.strftime("%Y-%m-%d %H:%M:%S"),
        # end_date: end_time.strftime("%Y-%m-%d %H:%M:%S"),
        # page_no: page,
        ...
        trades.each do |t|
          next if JingdongTrade.where(order_id: t['order_id']).exists?
