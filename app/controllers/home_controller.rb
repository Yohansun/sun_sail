      ...
      Trade.where(:id.in => params[:ids]).each_with_index do |trade, index|
        trade.orders.each do |td|
          unless index == 0
            ...
            name << {title: td.title, num: td.num, category: td.sku_properties_name.to_s} if num.length == 0
          else
            name << {title: td.title, num: td.num, category: td.sku_properties_name.to_s}
          end
        end
      end
