...
class SalesController < ApplicationController
  ...
  def product_analysis
    ...
    if @start_date && @end_date
      ...
      @trades = TaobaoTrade.where(account_id: current_account.id).between(created: 1.week.ago..Time.now).in(status: ["TRADE_FINISHED","FINISHED_L"])
      @old_trades = TaobaoTrade.where(account_id: current_account.id).between(created: 2.week.ago..1.week.ago).in(status: ["TRADE_FINISHED","FINISHED_L"])
    end
