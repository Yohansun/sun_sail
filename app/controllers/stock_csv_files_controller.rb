...
class StockCsvFilesController < ApplicationController
  ...
  def new
    @csv_file = StockCsvFile.last
    unless @csv_file.stock_in_bill_id.present?
  ...
  def create
    ...
    if (StockCsvFile.last.used == true rescue false) || StockCsvFile.last == nil
      if params[:stock_csv_file].present? && params[:stock_csv_file][:path].present?
        @csv_file = StockCsvFile.create(path: params[:stock_csv_file][:path], upload_user_id: current_user.id)
  ...
  def show
    @csv_file = StockCsvFile.find(params[:id])
    unless @csv_file.stock_in_bill_id.present?
  ...
  def destroy
    @csv_file = StockCsvFile.find(params[:id]).delete
    redirect_to "/stocks"
  end
end