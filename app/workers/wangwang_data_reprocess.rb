...
class WangwangDataReprocess
  ...
  def member_brief_info(start_date, end_date)
    ...
    @members.each do |m|
      ...
      created_trades = TaobaoTrade.where(:created.gte => start_date, :created.lt => end_date).where(:buyer_nick.in => inquired_today_and_yesterday)
      #下单人数
      daily_created_count = created_trades.try(:map, &:buyer_nick).count
      #下单金额
      ...
      #付款人数
      daily_paid_count = paid_trades.try(:map, &:buyer_nick).count
      #付款金额
      daily_paid_payment = paid_trades.try(:sum, :payment) || 0

      #当日询单，当日次日下单人数
      two_days_created_count = TaobaoTrade.where(:created.gte => start_date, :created.lt => (end_date + 1.day)).where(:buyer_nick.in => inquired_today).map(&:buyer_nick).to_a.count
      #当日询单，当日次日下单金额
      two_days_created_payment = TaobaoTrade.where(:created.gte => start_date, :created.lt => (end_date + 1.day)).where(:buyer_nick.in => inquired_today).sum(:payment) || 0
      #当日询单，当日次日均未下单（延迟一天统计）
      two_days_lost_count = inquired_today.count - two_days_created_count
      ...
      WangwangMemberContrast.create(
        ...
        two_days_lost_count:         two_days_lost_count,
        two_days_created_count:      two_days_created_count,
        two_days_created_payment:    two_days_created_payment
        )
    end
  end
