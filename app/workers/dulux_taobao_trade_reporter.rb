...
class DuluxTaobaoTradeReporter
  ...
  def perform(id)
    ...
    if current_user.has_role?(:cs) || current_user.has_role?(:admin)
      sheet1.row(1).concat ["订单号", "下单时间", "付款时间", "分流时间", "订单状态", "送货经销商", "买家地址-省", "买家地址-市", "买家地址-区", "买家地址", "买家姓名", "收货人手机/座机", "商品名", "数量", "商品标价",  "订单金额", "卖家优惠", "运费", "订单总金额", "调整金额", "买家旺旺",  "整单备注", "单品备注", "需要调色", "色号", "唯一码" ,"退款状态", "买家评价结果", "评价内容", "评价时间"]
    elsif current_user.has_role?(:seller) || current_user.has_role?(:logistic)
      sheet1.row(1).concat ["订单号", "下单时间", "付款时间", "分流时间", "订单状态", "送货经销商", "买家地址-省", "买家地址-市", "买家地址-区", "买家地址", "买家姓名", "收货人手机/座机", "商品名", "数量", "运费", "买家旺旺", "整单备注", "单品备注", "需要调色", "色号","唯一码","退款状态", "买家评价结果", "评价内容", "评价时间"]
    end
    trades.each_with_index do |trade, trade_index|
      ...
      trade_orders.each_with_index do |order, i|
        ...
        row_number += 1
        if current_user.has_role?(:cs) || current_user.has_role?(:admin)
          sheet1.update_row row_number, tid, created, pay_time, dispatched_at, taobao_status_memo, seller_name, receiver_state, receiver_city, receiver_district, receiver_address, receiver_name, receiver_mobile, title, num, order_price,  sum_fee, seller_discount, post_fee, total_fee, modify_payment, buyer_nick, trade_cs_memo, order_cs_memo, need_color, color_num, barcodes, refund_status_text,rate_result,rate_content,rate_created
        elsif current_user.has_role?(:seller) || current_user.has_role?(:logistic)
          sheet1.update_row row_number, tid, created, pay_time, dispatched_at, taobao_status_memo, seller_name, receiver_state, receiver_city, receiver_district, receiver_address, receiver_name, receiver_mobile, title, num, 0, buyer_nick, trade_cs_memo, order_cs_memo,need_color, color_num, barcodes, refund_status_text,rate_result,rate_content,rate_created
        end
        if trade_index.even?
