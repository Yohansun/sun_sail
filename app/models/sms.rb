...
class Sms
  ...
  def soap_transmit
    ...
    if Rails.env.production?
      ...
      response = client.request(:batch_send) do
        soap.body = "<loginName>#{loginName}</loginName><loginPswd>#{loginPswd}</loginPswd><mobiles>#{@account.settings.sms_test_mobile}</mobiles><content>#{self.content}</content><fixedTime></fixedTime><sendId></sendId><extid></extid>"
      end
      response.body[:batch_send_response][:out]
    end
  end
