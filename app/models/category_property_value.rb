...
class CategoryPropertyValue < ActiveRecord::Base
  ...
  def self.find_or_create_by_name_value(account_id, pname,pvalue)
    pname.strip!
    pvalue.strip!
    # category exist ?
    property = CategoryProperty.where(name: pname, account_id: account_id).first
    property = CategoryProperty.create(name:pname, account_id: account_id, status:CategoryProperty::STATUS_ENABLED, value_type:CategoryProperty::VALUE_TYPE_SINGLE) if property.nil?
    # value exist ?
