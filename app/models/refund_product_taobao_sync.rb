...
class RefundProductTaobaoSync < ECommerce::Synchronization::Base
  ...
  alias_columns buyer_nick: 'buyer_name',created: "refund_time",total_price: "total_fee"

  def initialize(trade_source_id,default_attrs={})
    @trade_source = TradeSource.find(trade_source_id)
    @default_attributes = {account_id: @trade_source.account_id,ec_name: "TaobaoTrade",is_location: false}.merge(default_attrs.reject {|k,v| v.nil?})
  ...
  def parsing
    super && (@page_no+=1) while page_no <= page_count
  end

  def taobao_trade(tid)
    TaobaoTrade.unscoped.where(:tid => tid).first
  end

  # refund_orders_attributes
  def refund_order_attributes(refund)
    taobao_trade = taobao_trade(refund["tid"])
    ...
    handle_exception(response: refund) do
      ...
      refund_price = refund["refund_fee"]
