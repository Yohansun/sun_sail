...
class Sale < ActiveRecord::Base
  ...
  def all_trade_fee(start_time, end_time)
    all_trades = Trade.where("$and" => [{:created.gte => start_time.utc }, {:created.lte => end_time.utc}])
    all_money = all_trades.inject(0) { |sum, trade| sum + trade.calculate_fee }
    all_money.to_i
  end

  def paid_trade_fee(start_time, end_time)
    paid_trades = Trade.where("$and" => [{:created.gte => self.start_at.to_time }, {:pay_time.gte => start_time.utc }, {:pay_time.lte => end_time.utc }])
    if paid_trades
