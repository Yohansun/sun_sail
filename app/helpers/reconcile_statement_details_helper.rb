...
module ReconcileStatementDetailsHelper
  def judge_money_type_th(money_type)
    ...
    case money_type
      ...
      when 'base_service_fee'
        ...
        info_array = [trade.total_fee, ((trade.total_fee - trade.post_fee - refund_fee)*base_service_point).round(2)]
      when 'store_service_award'
        refund_fee = calculate_refund_fee(trade)
        store_service_point = 0.04 ##not defined, later
        info_array = [trade.total_fee, ((trade.total_fee - trade.post_fee - refund_fee)*store_service_point).round(2)]
      when 'staff_award'
        refund_fee = calculate_refund_fee(trade)
        staff_award_point = 0.02 ##not defined, later
        info_array = [trade.total_fee, ((trade.total_fee - trade.post_fee - refund_fee)*staff_award_point).round(2)]
      when 'taobao_cost'
        taobao_cost_point = 0.02 ##not defined, later
        info_array = [trade.total_fee, (trade.total_fee*taobao_cost_point).round(2)]
      when 'collecting_postfee'
        seller_post_fee = 0  ##not defined, later
        info_array = [trade.total_fee, (trade.sum_fee + trade.post_fee), seller_post_fee]
      when 'adjust_amount'
        info_array = [trade.total_fee, trade.modify_payment]
    end
    info_array
  end
  ...
  def money_type_text(money_type)
    case money_type
      when 'alipay_revenue'
        "支付宝收入"
      when 'postfee_revenue'
