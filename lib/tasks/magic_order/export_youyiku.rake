...
task :export_youyiku => :environment do
  ...
  trades_total_count = account.trades.count
  trades_sheets_count = trades_total_count/20000+1
  (0..trades_sheets_count).each do |sheet_count|
    trades = account.trades.order_by(:created.asc).skip(sheet_count*20000).limit(20000)
    ...
    trades.each_with_index do |trade, trade_index|
      ...
      trade_orders.each_with_index do |order, order_index|
        ...
        order.skus_info_with_offline_refund.each_with_index do |info, index|
          skus_outer_id_infos += " #{info.fetch(:outer_id)}"
          skus_number_infos += info.fetch(:number).to_i
        end

        body << skus_outer_id_infos
        body << skus_number_infos
    ...
    file = "#{Rails.root}/data/youyiku/#{(sheet_count+1)*20000}.xls"
    book.write file
  end
end