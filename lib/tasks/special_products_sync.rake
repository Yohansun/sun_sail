...
task :special_products_sync => :environment do
    ...
    barea = district || city || state
    if barea
    	barea.sellers.where("seller_id not in (?)", [1791,1792,1793,1794]).each {|seller| seller.stock_products.where(product_id: product.id).delete_all}
    else
