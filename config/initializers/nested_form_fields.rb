module ActionView::Helpers
  ...
  class FormBuilder
    ...
    def add_nested_fields_link(association,*args,&block)
      options = args.extract_options!
      args[0] ||= "Add #{association.to_s.singularize.humanize}"
      args[1] ||= ""
      options[:data]  = { association_path: association_path(association.to_s) }
      options[:class] = (options[:class].to_s) + " add_nested_fields_link"
      @template.link_to(*args,options,&block)
    end
